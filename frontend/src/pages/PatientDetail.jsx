...
function PatientDetail() {
  ...
  const resepRefs = useRef({})

  const handleCapture = async (rekam_id = null) => {
    const node = rekam_id ? resepRefs.current[rekam_id] : resepObatRef.current
    if (!node) return
  ...
  const handlePrint = async (rekam_id = null) => {
    const node = rekam_id ? resepRefs.current[rekam_id] : resepObatRef.current
    if (!node) return
      ...
      win.document.write(`
        <html>
          <head>
            ...
            <style>
              body { margin:0; padding:0; text-align:center; font-family:sans-serif; display:flex; flex-direction:column; height:100vh; }
              img { max-width:90%; height:auto; margin-top:20px; }
              .signature { text-align:left; margin:60px 0 0 60px; font-size:16px; font-weight:bold; color:#000; }
              .signature-line { display:block; width:200px; border-top:1px solid #000; margin-bottom:5px; }
              .recipe-data { flex:1; }
            </style>
          </head>
          <body>
  ...
  return (
    <div className='p-6 lg:ms-70'>
      ...
      <div className='mt-10'>
        <div className='flex flex-col gap-4 bg-blue-100 border border-blue-300 rounded-md p-6'>
          <div className='flex gap-6 items-center'>
            ...
            <div className='flex flex-col gap-3'>
              ...
              <p className='px-3 py-1 bg-blue-400 text-white rounded-full w-fit'>
                {singleRekamMedis?.diagnosa || 'Belum ada diagnosa'}
              </p>
            </div>
          </div>
        ...
        <div className='p-1 bg-gray-200 grid grid-flow-col text-nowrap overflow-x-auto justify-items-center mt-6 rounded-md gap-x-2'>
            ...
            <div className='grid grid-cols-1 gap-y-3'>
              ...
              <div className='p-6 border border-gray-300 rounded-md flex flex-col gap-5'>
                ...
                <div className='px-5 py-3 border border-gray-300 rounded-md flex flex-col gap-3'>
                  ...
                  <p ref={resepObatRef} className='text-black'>
                    <span className='font-semibold'>Resep:</span> {singleRekamMedis?.resep_obat || 'belum ada'}
                  </p>
                  ...
                  <div key={rm.rekam_id} className='p-4 border border-gray-300 rounded-md bg-white'>
                    <div className='mb-3'>
                      ...
                      <p className='text-gray-500'>
                        {new Date(rm.dibuat_pada || '2025-01-01').toLocaleDateString('id-ID', {
                          year: 'numeric',
                          month: 'numeric',
                          day: 'numeric',
                        })}
                    ...
                    <div className='flex flex-col gap-1 mb-3 pb-3 border-b border-gray-300'>
                      ...
                      <p className='text-gray-500'>Suhu: {rm.suhu || 'belum ada'}</p>
                      <p className='text-gray-500'>Berat Badan: {rm.berat_badan || 'belum ada'}</p>
                    </div>

                    <p
                      ref={(el) => (resepRefs.current[rm.rekam_id] = el)}
                      className='text-black'
                    >
                      <span className='font-semibold'>Resep:</span> {rm.resep_obat || 'belum ada'}
                    </p>

                    <div className='grid grid-cols-1 lg:grid-cols-2 w-full gap-2 mt-2'>
                      <button onClick={() => handleCapture(rm.rekam_id)} className='bg-emerald-500 text-white p-2 rounded-md flex items-center justify-center gap-2'>
                        <Download /> <p>Download Resep Obat</p>
                      </button>
                      <button onClick={() => handlePrint(rm.rekam_id)} className='bg-blue-500 text-white p-2 rounded-md flex items-center justify-center gap-2'>
                        <Printer /> <p>Print Resep Obat</p>
                      </button>
                    </div>
                  </div>
